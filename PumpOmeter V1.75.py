...
par = input(" Elija una opcion de la lista (a,b,c,d,e): ")
if par == 'a':
    PAR = 'BUSD'
elif par == 'b':
    PAR = 'USDT'
    ...
    while anclajestot > 0:
        ...
        while cantrefresh1 > 0:
            ...
            for each in sortdict:
                ...
                if bloqueador < 5 and bloqueadorb < 10:
                    if float(sortdict[each]) >= 0.75 and float(sortdict[each]) < 1.25:
                        mixer.init()
                        mixer.music.load("Alarm04.wav")
                        mixer.music.play()
                        print(" -------------------------")
                        print(" " + each + " subio un poco!!")
                        print(" -------------------------")
                        if whats == 's':
                            ...
                            for cell in sheet['A']:
                                if cell.value == each:
                                    pg.hotkey('shift', 'enter')
                                    pg.write("Acumulativo: " + str(float(sheet[f"G{cell.row}"].value)) + "%  " + "R" + str(cantrefresh1))
                                    pg.press('enter')
                                    bloqueador = bloqueador + 1
                if bloqueadorb < 10:
                    if float(sortdict[each]) >= 1.25 and float(sortdict[each]) < 2.75:
                        mixer.init()
                        ...
                        if whats == 's':
                            ...
                            for cell in sheet['A']:
                                if cell.value == each:
                                    ...
                                    pg.write("Acumulativo: " + str(float(sheet[f"G{cell.row}"].value)) + "%  " + "R" + str(cantrefresh1))
                                    pg.press('enter')
                                    bloqueadorb = bloqueadorb + 1
                if float(sortdict[each]) >= 2.75 and float(sortdict[each]) < 5:
                    mixer.init()
                    ...
                    print(" -------------------------")
                    if whats == 's':
                        pg.write("*"+each+"*" + " subio " + "*"+str(float(sortdict[each]))+"*" + "% :-p ")
                        for cell in sheet['A']:
                            if cell.value == each:
                                pg.hotkey('shift', 'enter')
                                pg.write("Acumulativo: " + str(float(sheet[f"G{cell.row}"].value)) + "%  " + "R" + str(cantrefresh1) )
